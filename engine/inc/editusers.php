...
HTML;
		...
		if (count($temp_array)) {
			...
			foreach ( $temp_array as $value ) {
				...
				if ( filter_var( $value1[0], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ) {
					$value1[0] = filter_var( $value1[0] , FILTER_VALIDATE_IP, FILTER_FLAG_IPV4);
				} elseif ( filter_var( $value1[0], FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) ) {
					$value1[0] = filter_var( $value1[0], FILTER_VALIDATE_IP, FILTER_FLAG_IPV6);
				} else $value1[0] = false;
	
				if( $value1[0] ) {
					if( intval($value1[1]) ) {
						$allowed_ip[] = trim($value1[0])."/".intval($value1[1]);
					} else $allowed_ip[] = trim($value1[0]);
				}
			}
	...
	if ($_GET['popup'] == "yes") {
		...
		die( "<body><script>window.close();</script>".$lang[user_ok]." ".$lang[user_delok_1]."</body>" );
