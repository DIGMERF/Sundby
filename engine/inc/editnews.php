...
if( $action == "list" ) {
	...
	if( count($search_cat) ) {
		...
		if( count($search_cat) ) {
			...
			$w_cat = "c.cat_id IN (".implode(",", $search_cat).")";
	...
	$db->query( "SELECT p.id, p.date, p.title, p.category, p.autor, p.alt_name, p.comm_num, p.approve, p.fixed, e.news_read, e.votes, e.user_id, e.need_pass FROM " . PREFIX . "_post p {$cat_join}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) " . $where . " {$group_by}ORDER BY " . $order_by . " LIMIT {$start_from},{$news_per_page}" );
	// Prelist Entries
...
HTML;
	...
	// End prelisting
	$result_count = $db->super_query( "SELECT COUNT(*) as count FROM " . PREFIX . "_post p {$cat_join}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) " . $where. "{$group_by}");
...
HTML;
	...
	$db->query( "SELECT id, autor, date, xfields, title, category, approve, tags, news_id FROM " . PREFIX . "_post LEFT JOIN " . PREFIX . "_post_extras ON (" . PREFIX . "_post.id=" . PREFIX . "_post_extras.news_id) WHERE id = '$id'" );
	...
	if( $item_db[0] ) {
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				if ($item_db[6]) $db->query( "UPDATE " . PREFIX . "_post_extras SET related_ids='', allow_rate='$allow_rating', votes='$add_vote', disable_index='$disable_index', access='$group_regel', editdate='$added_time', editor='{$member_id['name']}', reason='$editreason', view_edit='$view_edit', disable_search='{$disable_search}', need_pass='{$need_pass}' WHERE news_id='$item_db[0]'" );
