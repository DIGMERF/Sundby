...
function build_js($js) {
	...
	if ($config['js_min']) {
		...
		$js_array[] = "<script src=\"engine/classes/min/index.php?charset={$config['charset']}&amp;g=admin&amp;v=26\"></script>";

		if ( count($js) ) $js_array[] = "<script src=\"engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $js)."&amp;v=26\" defer></script>";
		...
		foreach ($js as $value) {
			...
			$js_array[] = "<script src=\"{$value}?v=26\"{$defer}></script>";
...
function build_css($css) {
	...
	if ($config['js_min']) {
		...
		return "<link href=\"engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $css)."&amp;v=26\" rel=\"stylesheet\" type=\"text/css\">";

	} else {

		foreach ($css as $value) {
		
			$css_array[] = "<link href=\"{$value}?v=26\" rel=\"stylesheet\" type=\"text/css\">";
