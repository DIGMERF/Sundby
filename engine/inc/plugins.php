...
<dleplugin>
	...
	<phpdelete><![CDATA[{$row['phpdelete']}]]></phpdelete>{$x}
</dleplugin>
HTML;
...
if( $_REQUEST['action'] == "doadd" OR $_REQUEST['action'] == "doedit" ) {
	...
	if( $_REQUEST['action'] == "doadd" ) {
		...
		if($_POST['phpinstall']) {
			eval($_POST['phpinstall']);
		}
		
		if($_POST['phpenable'] AND $plugin_active) {
			eval($_POST['phpenable']);
		}
...
HTML;
		...
		$entries .= "
			<tr>
			 <td class=\"{$border}cursor-pointer\" onclick=\"document.location = '?mod=plugins&action=edit&id={$row['id']}'; return false;\"><div class=\"media-list\"><div class=\"media-left\"><img src=\"{$row['icon']}\" class=\"img-lg section_icon\"></div><div class=\"media-body\"><h6 class=\"media-heading text-semibold\">{$row['name']}{$row['version']}{$status}{$plugin_error}</h6><span class=\"text-muted text-size-small\">{$row['description']}</span><span id=\"upgrade{$row['id']}\" class=\"label label-success position-right text-size-small\"></span></div></div></td>
