	...
	if ( $row['id'] AND  $perm ) {
		...
		if( stripos( $tpl->copy_template, "{next-url}" ) !== false OR stripos( $tpl->copy_template, "{prev-url}" ) !== false) {
			...
			if( !is_array($prev_next) ) {
				...
				$row_link = $db->super_query( "SELECT id, date, category, alt_name FROM " . PREFIX . "_post WHERE category = '{$row['category']}' AND date >= '{$row['date']}'{$where_date} AND id != '{$row['id']}' AND approve = '1' ORDER BY date ASC LIMIT 1" );
				...
				$row_link = $db->super_query( "SELECT id, date, category, alt_name FROM " . PREFIX . "_post WHERE category = '{$row['category']}' AND date <= '{$row['date']}'{$where_date} AND id != '{$row['id']}' AND approve = '1' ORDER BY date DESC LIMIT 1" );
		...
		if( $empty_full ) {
			$allcontent = trim(str_replace("&amp;amp;", "&amp;", htmlspecialchars(strip_tags( $row['full_story']." ".$all_xf_content ), ENT_COMPAT, $config['charset'] )));
		} else {
			$allcontent = trim(str_replace("&amp;amp;", "&amp;", htmlspecialchars(strip_tags( $row['full_story']." ".$row['short_story']." ".$all_xf_content ), ENT_COMPAT, $config['charset'] )));
		}
		...
		if( $allcontent ) $social_tags['description'] = preg_replace('/\s+/u', ' ', $allcontent);
