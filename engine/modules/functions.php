...
function build_js($js, $config) {
	...
	if ($config['js_min']) {
		...
		$js_array[] = "<script src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;g=general&amp;v=26\"></script>";
		...
		$js_array[] = "<script src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $js)."&amp;v=26\" defer></script>";
		...
		foreach ($js as $value) {
			if($i > 0) $defer =" defer";
			$js_array[] = "<script src=\"{$config['http_home_url']}{$value}?v=26\"{$defer}></script>";
			$i++;
		}
...
function build_css($css, $config) {
	...
	if ($config['js_min'] AND count($css) ) {
		...
		return "<link href=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $css)."&amp;v=26\" rel=\"stylesheet\" type=\"text/css\">";
		...
		foreach ($css as $value) {
			...
			$css_array[] = "<link href=\"{$config['http_home_url']}{$value}?v=26\" rel=\"stylesheet\" type=\"text/css\">";
